...
class DomainService:
    ...
    def __parse_domain_reg_date(self, domain_name: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                self.__logger.error(f'Received invalid response from remote URL {self.__env.whois_url}. Expected 200, received {response.status_code}')
            else:
                ...
                self.__logger.info(f'Parsed domain info from whois.com. Domain {domain_name} was registered on {registration_date}')
    ...
    def get_age_of_domain(self, domain_name: str) -> int:
        ...
        It checks if the domain name is already cached, if yes it returns from the cache else it parses the registration date from whois.com and calculates the doamin age in days
        :param domain_name : domain name whose age needs to be retrived
        """
        reg_date = self.__domain_cache_handler.lookup(domain_name)

        if reg_date:
            self.__logger.info(f'Domain registration details found in local cache. Domain {domain_name} was registered on {reg_date}')
        else:
